						...
						<div class="col-xs-12 col-sm-12 col-md-10 col-lg-10">
							<div class="card shadow mb-4">
								<div class="card-header py-3">
									<h6 class="m-0 font-weight-bold text-primary">Laporan Absensi Tentor</h6>
								</div>
								<div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table id="dataTable" border="0" class="table table-bordered">
											...
											<tbody>
                                                <?php
                                                $listAbsen = mysqli_query($db, "SELECT idTentor, namaTentor, paket, COUNT(absensi) 'jumlah' FROM `absensi_tentor` WHERE absensi = 'Hadir' GROUP BY idTentor, paket");
                                                if (isset($_GET['bulan'])) $listAbsen = mysqli_query($db, "SELECT idTentor, namaTentor, paket, COUNT(absensi) 'jumlah' FROM `absensi_tentor` WHERE absensi = 'Hadir' AND MONTH(tglAbsen) = '$_GET[bulan]' GROUP BY idTentor, paket");
                                                if (isset($_GET['bulan']) && isset($_GET['tahun'])) $listAbsen = mysqli_query($db, "SELECT idTentor, namaTentor, paket, COUNT(absensi) 'jumlah' FROM `absensi_tentor` WHERE absensi = 'Hadir' AND MONTH(tglAbsen) = '$_GET[bulan]' AND YEAR(tglAbsen) = '$_GET[tahun]' GROUP BY idTentor, paket");
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?= $list['paket']; ?></td>
                                                    <td><?= $list['jumlah']; ?></td>
                                                    <td><a href="daftar-absen-tentor.php?id=<?= $list['idTentor'] . '&paket=' . $list['paket']; ?>">Lihat Absen</td>
                                                </tr>
                                                <?php
                                                    }
                                                }
